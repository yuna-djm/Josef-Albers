class Mover {
  ...
  show() {
    ...
    // strokeWeight(this.pixelW - 20);
    // line(this.curX1, this.curY1, this.curX2, this.curY2);

    strokeCap(PROJECT);
    strokeWeight(random(this.pixelW - 10, this.pixelW));
    stroke(this.clrs[1]);
    line(this.curX1, this.curY1, this.curX2, this.curY2);
  }
  ...
  update() {
    ...
    if (
      this.curX2 <= this.w / 2 ||
      this.curX2 >= width - this.w / 2 ||
      this.curY2 <= this.w / 2 ||
      this.curY2 >= height - this.w / 2
    ) {
      this.spawnBody();
      this.isDone = true;
      return;
    ...
    if (this.timings[4] < this.timer && !this.isDone) {
      this.init();
    }
  ...
  spawnBody() {
    let body = Bodies.rectangle(
      ...
      this.pixelW,
      {
        ...
        frictionAir: 0,
        render: {
          fillStyle: color(random(palette)).toString("#rrggbb"),
          colorShift: [random(0.5, 1.05), random(0.5, 1.05), random(0.5, 1.05)],
          colorShift2: [random(0.5, 1), random(0.5, 1), random(0.5, 1)],
        },
      }
    );
